...
checkOS() {
    ...
    # Check for Windows-specific versions (WSL)
    if [ "$os_type" = "Linux" ] && [[ "$os_version" == "2004" || "$os_version" == "10" || "$os_version" == "11" ]]; then
        is_windows=1
...
checkCPU() {
    ...
    case "$CPUArch" in
        ...
        x86_64|amd64)
            ...
            ;;
        *)
            echo "Unsupported CPU architecture: $CPUArch"
            arch=_unknown
            ;;
    esac
}
...
function Check_DNS_1()
{
    ...
    local address=$(echo "$resultdns" | awk '/^Address: / { print $2 ; exit }')

    if [ -z "$address" ]; then
        echo "0"  # No address found
        return
    fi

    local server_ip=$(echo "$resultdns" | awk '/^Server:/ { getline; print $2 }')

    echo $(Check_DNS_IP "$address" "$server_ip")
}
...
function Check_Proxy()
{
    ...
    if [ "$1" = "4" ]; then
        resultip=$(ifconfig | grep 'inet ' | grep -v '127.0.0.1' | awk '{print $2}')
        resultrealip=$(curl -s -4 ip.sb)
    elif [ "$1" = "6" ]; then
        resultip=$(ifconfig | grep 'inet6 ' | grep -v '::1' | awk '{print $2}')
        resultrealip=$(curl -s -6 ip.sb)
    else
        echo "2"
        return
    fi

    if [ -z "$resultip" ] || [ -z "$resultrealip" ]; then
        echo "3"
        return
    fi

    local IFS=$'\n'
    for ip in $resultip; do
        if [ "$1" = "4" ] && [ "$(Check_Private_IP "$ip")" = "1" ]; then
            echo "$ip"
            return
        elif [ "$1" = "6" ] && [ "$(Check_Private_IP "$ip")" = "0" ]; then
            echo "3"
            return
        fi
    done
...
function MediaUnlockTest_YouTube_Premium() {
    ...
    local region=$(echo $tmpresult | grep "contentRegion" | sed 's/.*"contentRegion"//' | cut -f2 -d'"')
...
function MediaUnlockTest_YouTube_CDN() {
    ...
    # echo "YouTube Region: curl $useNIC $usePROXY $xForward -${1} ${ssll} -sS --max-time 10 \"https://redirector.googlevideo.com/report_mapping\""
...
function MediaUnlockTest_NetflixCDN() {
    ...
    # echo "Netflix Preferred CDN: curl $useNIC $usePROXY $xForward -${1} ${ssll} -s --max-time 10 \"https://api.fast.com/netflix/speedtest/v2?https=true&token=YXNkZmFzZGxmbnNkYWZoYXNkZmhrYWxm&urlCount=1\""
    # echo "Netflix Preferred CDN: tmpresult is $tmpresult"
    if [ -z "$tmpresult" ]; then
        echo -n -e "\r Netflix Preferred CDN:\t\t\t${Font_Red}Failed${Font_Suffix}\n"
        return
    elif echo "$tmpresult" | grep -q '>403<'; then
        echo -n -e "\r Netflix Preferred CDN:\t\t\t${Font_Red}Failed (IP Banned By Netflix)${Font_Suffix}\n"
        return
    fi

    local CDNAddr=$(echo "$tmpresult" | sed -n 's/.*"url":"\([^"]*\).*/\1/p' | cut -d'/' -f3)
    # echo "Netflix Preferred CDN: CDNAddr is $CDNAddr"
    if [ -z "$CDNAddr" ]; then
        echo -n -e "\r Netflix Preferred CDN:\t\t\t${Font_Red}Failed (CDN Address Not Found)${Font_Suffix}\n"
        return
    fi

    local CDNIP
    if [ "$1" = "6" ]; then
        CDNIP=$(host -t AAAA "$CDNAddr" | awk '/has IPv6 address/ {print $NF; exit}')
    else
        CDNIP=$(host -t A "$CDNAddr" | awk '/has address/ {print $NF; exit}')
    fi
    # echo "Netflix Preferred CDN: CDNIP is $CDNIP"
    ...
    local CDN_ISP=$(curl $useNIC $xForward --user-agent "${UA_Browser}" -s --max-time 20 "https://api.ip.sb/geoip/$CDNIP" | sed -n 's/.*"isp":"\([^"]*\).*/\1/p')
    # echo "Netflix Preferred CDN: CDN_ISP is $CDN_ISP"
    local iata=$(echo "$CDNAddr" | cut -f3 -d"-" | sed 's/...$//' | tr '[:lower:]' '[:upper:]')
    # echo "Netflix Preferred CDN: iata is $iata"

    local location=""
    # echo "Netflix Preferred CDN: IATACode is $IATACode"
    # echo "Netflix Preferred CDN: IATACode2 is $IATACode2"
    if echo "$IATACode" | grep -q "$iata"; then
        location=$(echo "$IATACode" | awk -F'|' "/$iata/ {print \$1}" | sed 's/^ *//')
    elif echo "$IATACode2" | grep -q "$iata"; then
        location=$(echo "$IATACode2" | awk -F',' "/$iata/ {print \$2}" | sed 's/^ *//' | sed 's/.*/\L&/; s/[a-z]*/\u&/g')
    fi
    # echo "Netflix Preferred CDN: location is $location"

    if [ -n "$location" ]; then
        if [ "$CDN_ISP" = "Netflix Streaming Services" ]; then
        ...
        echo -n -e "\r Netflix Preferred CDN:\t\t\t${Font_Red}No ISP Info Found${Font_Suffix}\n"
    else
        echo -n -e "\r Netflix Preferred CDN:\t\t\t${Font_Red}Unable to determine location${Font_Suffix}\n"
    fi
}
...
function MediaUnlockTest_DiscoveryPlus() {
    ...
    local baseApiUrl=$(echo "$tmpresult" | sed -n 's/.*"baseApiUrl"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p')
    local realm=$(echo "$tmpresult" | sed -n 's/.*"realm"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p')
    ...
    local fakeDeviceId=$(openssl rand -hex 16 | md5 | cut -f1 -d' ')
    ...
    local token=$(echo "$tmpresult1" | sed -n 's/.*"token"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p')
    if [ -z "$token" ]; then
    ...
    local region=$(echo "$tmpresult2" | sed -n 's/.*"mainTerritoryCode"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | tr '[:lower:]' '[:upper:]')
...
function GameTest_MathsSpot() {
    ...
    local fakeUAId=$(generate_random_string 21)
    local fakeSessId=$(generate_random_string 21)
    local fakeFesessId=$(generate_random_string 21)
    local fakeVisitId=$(generate_random_string 21)
...
function Wikipedia_Editable(){
    ...
    # echo "Wikipedia Editability: curl $useNIC $usePROXY $xForward -${1} ${ssll} -s --max-time 10 \"https://zh.wikipedia.org/w/index.php?title=Wikipedia%3A%E6%B2%99%E7%9B%92&action=edit\""
...
function MediaUnlockTest_AMCPlus() {
    ...
    local region=$(echo "$urlEffective" | awk -F'/' '{print $NF}' | tr '[:upper:]' '[:lower:]' | sed -e 's/^./\u&/' -e 's/ ./\u&/g')
    if [ -n "$isBlocked" ]; then
...
function ScriptTitle() {
    if [[ "$language" == "e" ]]; then
        ...
        echo -e "${Font_Green}FreeBSD Fork${Font_Suffix} ${Font_Yellow}https://github.com/cngeeks/RegionRestrictionCheck ${Font_Suffix}"
        echo -e "${Font_Purple}Supporting OS: FreeBSD 9+, CentOS 6+, Ubuntu 14.04+, Debian 8+, MacOS, Android (Termux), iOS (iSH)${Font_Suffix}"
        echo ""
        ...
        echo -e "${Font_Green}FreeBSD版本${Font_Suffix} ${Font_Yellow}https://github.com/cngeeks/RegionRestrictionCheck ${Font_Suffix}"
        echo -e "${Font_Purple}脚本适配OS: FreeBSD 9+, CentOS 6+, Ubuntu 14.04+, Debian 8+, MacOS, Android (Termux), iOS (iSH)${Font_Suffix}"
        echo ""
        echo -e " ** 测试时间: $(date)"
        echo ""
    fi
